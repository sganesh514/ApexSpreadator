...
class OptionsSelector:
    ...
    def select_spread(
        ...
        Parameters:
        - direction: 'BULLISH' (Call spread) or 'BEARISH' (Put spread)
        - target_tp: Target take-profit price (structural swing point)
        - target_sl: Invalidation point (past the consolidation zone)
        - options_chain: Optional list of option contracts with bid/ask prices
        
        Returns:
        - (spread_object, status_reason)
        """
        ...
        if options_chain:
            ...
            if not valid_options:
                ...
                self._log_rejection(expiration, symbol, direction, underlying_price, 0, 0, 0, 0, status)
                return None, status
